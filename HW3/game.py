...
def whoIsFirst(s):
    ...
    choice = input("Would you like to be black (b) or white (w)? ")
    while True:
        if choice == "b":
            return setHumanComputer(s, BLACK, WHITE)
        elif choice == "w":
            return setHumanComputer(s, WHITE, BLACK)
        else:
            choice = input("Please enter b or w ")

def setHumanComputer(s, h, c):
    global HUMAN, COMPUTER
    HUMAN, COMPUTER = h, c
    s[2] = BLACK
    return s
...
# Reads, enforces legality and executes the user's move.
    ...
    while flag:
        printState(s)
        move=input("To make your move enter a two digits number, the first number is row and the second is column" "\n" \
        "For example: if you want to choose the first row and the third column enter 13" "\n"\
        "Enter your next move: ")
        if move == "options":
            print(legalMoves(s))
        elif isLegal(int(move), s) ==False:
            print("Illegal move.")
        else:
            flag=False
            makeMove(int(move),s)
...
def findBracket(square, s, direction):
    # print("Looking for bracket from ", square, "in direction", direction)
    ...
    # if not s[0][bracket] in (EMPTY):
        # print("FOUND BRACKET: ", square, direction,
        #       opp, bracket, s[2] == WHITE, s[2], WHITE)
